...
void Eboot::readHeader(){
    SceUID fp = sceIoOpen(this->path.c_str(), PSP_O_RDONLY, 0777);
...
void Eboot::readFile(void* dst, unsigned offset, unsigned size){
    SceUID src = sceIoOpen(this->path.c_str(), PSP_O_RDONLY, 0777);
    sceIoLseek(src, offset, PSP_SEEK_SET);
    sceIoRead(src, dst, size);
    sceIoClose(src);
}
