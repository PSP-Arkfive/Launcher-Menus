...
SfoInfo UMD::getSfoInfo(){
    ...
    if (sfo_buffer){
        ...
        SceUID fp = sceIoOpen(UMD_SFO_PATH.c_str(), PSP_O_RDONLY, 0777);
        sceIoRead(fp, sfo_buffer, size);
        sceIoClose(fp);
...
void UMD::loadAVMedia(){
    ...
    if (common::fileExists(UMD_GAME_SND0_PATH)){
        ...
        this->at3_size = size;
        // read file
        SceUID src = sceIoOpen(UMD_GAME_SND0_PATH, PSP_O_RDONLY, 0777);
        sceIoRead(src, this->snd0, size);
        sceIoClose(src);
    }
    ...
    if (common::fileExists(UMD_GAME_ICON1_PATH)){
        ...
        // read file
        SceUID src = sceIoOpen(UMD_GAME_ICON1_PATH, PSP_O_RDONLY, 0777);
        sceIoRead(src, this->icon1, size);
        sceIoClose(src);
    }
}
